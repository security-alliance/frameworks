...
| Data | Source |
| ------ | -------- |
| Memory and disk images | The device, before rebuild |
| Process, persistence, and install history | Endpoint tooling, local logs |
| Authentication and session logs | Identity provider, code host, cloud |
...
Preserve evidence before rebuilding. See [Forensic Readiness](/incident-management/forensic-readiness) for
chain of custody and retention design, which only helps if it is already in place when the incident starts.
