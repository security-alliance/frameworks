...
- **npm registry attack (2025).** The largest npm supply chain attack to date. Attackers phished maintainer credentials
  via spoofed emails from `support@npmjs.help` and injected a multi-chain crypto drainer into 18 packages including
  `chalk`, `debug`, and `ansi-styles` (2.6 billion combined weekly downloads). The payload hooked `window.ethereum` to
  intercept wallet calls and overrode `fetch`/`XMLHttpRequest` to scan for blockchain addresses. Detected within
  roughly two hours due to a poorly chosen obfuscator that crashed CI pipelines, limiting actual theft to around $500.
