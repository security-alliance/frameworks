...
survives a misconfigured client. Cargo has no scoped namespaces, so a crate name is reserved by publishing a stub
under it; Go has no name registry at all, since module paths are import paths rooted in a domain the organization
already controls.

**Route by name, and replace rather than extend.** Internal scopes should resolve only from the internal registry.
The exact mechanism differs by ecosystem, and the distinction that matters is whether a setting *replaces* the
resolution path or *adds* to it; the additive form is the one that is exploitable, because it leaves the public
source reachable for a name the internal registry was supposed to own.

| Ecosystem | Replaces resolution | Adds a second source (exploitable) |
| --------- | -------------------- | ----------------------------------- |
| **pip** | `--index-url` (or `PIP_INDEX_URL`) points every lookup at the internal index. | `--extra-index-url` searches the public index too, with version precedence deciding the winner. |
| **npm** | Binding a scope to a registry (`npm config set @org:registry <url>`) means `@org/*` never resolves elsewhere. | An unscoped internal name with no registry binding falls through to the default registry. |
| **Go** | `GOPRIVATE` (a glob pattern of module path prefixes) marks internal modules as private, so `go get` skips the public proxy and checksum database for them and fetches directly from the source instead. | A private module path left out of `GOPRIVATE` resolves through `GOPROXY` and the public checksum database like any other module. |
| **Cargo** | A `[source]` replacement in `.cargo/config.toml` (`replace-with`) reroutes crates.io itself to the internal registry for every dependency. | Declaring the internal registry only in `[registries]` and referencing it per-dependency leaves the crates.io source active for anything not explicitly redirected. |

Solidity has no registry of its own to route. Foundry pins dependencies to specific git commits via submodules, so
resolution is fixed by the commit hash rather than by a name lookup, which is why dependency confusion does not apply
to it the way it applies to a name-based resolver. Hardhat projects that pull dependencies through npm inherit npm's
routing behavior above, and should be configured the same way. See
[Dependency Awareness](/supply-chain/dependency-awareness) for how each ecosystem's lockfile fits alongside routing.
...
- [Go private modules](https://go.dev/ref/mod#private-modules): marking module paths private with `GOPRIVATE` and
  routing them around the public proxy and checksum database
- [Cargo source replacement](https://doc.rust-lang.org/cargo/reference/source-replacement.html): rerouting crates.io
