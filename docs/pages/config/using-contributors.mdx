...
This page demonstrates how to use the centralized contributors database. Instead of specifying all the contributor
details in each file, you can now simply reference contributors by their ID.
...
When using the role-based format, contributors will be displayed in sections based on their roles. The standard roles
are:

- `wrote`: People who authored the content. A new page, a whole section, or a substantial rewrite.
- `reviewed`: People who read the full page and signed off on it in a PR review, judging structure, scope, coverage,
  and accuracy together.
- `fact-checked`: People who verified specific claims against primary sources and corrected, qualified, or sourced
  them. The scope is the individual statements, not the page as a whole, and it is not a fallback list for edits too
  small to count as authorship.

A reviewer answers "is this page good?"; a fact-checker answers "is this statement true?".

This creates a clear attribution system showing who contributed to each page in what capacity. The full rules,
including which role to pick for your own contribution, are in the
[contributor guide](/contribute/contributing#attribution-and-contributor-roles).
