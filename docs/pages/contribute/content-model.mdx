...
| `contributors` | Roles `wrote`, `reviewed`, `fact-checked` with slugs defined in the contributors database; first-time authors also register in `contributors.json`. Which role a given contribution belongs under is defined in the [contributing guide](/contribute/contributing#attribution-and-contributor-roles) |

Do not rewrite historical contributor attribution without clear evidence it is wrong, and never remove or reorder
contributors already listed on a page.
...
- `## What this framework covers` (preferred for a top-level framework overview)
- `## What this section covers` (preferred for a nested subsection overview)
