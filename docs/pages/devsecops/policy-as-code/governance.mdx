...
strictest thresholds a team can operate. Low reachability with straightforward rollback takes
...
derivation belongs to the adopting organization, which is why the reasoning behind a threshold
belongs in the policy repository alongside the threshold itself.
...
enforced version is a readable property. This also follows from the standard's definition of a
...
- [ ] Every rule **must** carry an id, a named owner, and a class
- [ ] Rules **should** live in one place and ship as versioned, signed bundles
- [ ] Every rule **must** have a passing and a failing test fixture, run in CI
- [ ] New rules **should** go dry-run, then warn, then block, each with an enforcement date set
  at creation
- [ ] Unclassified rules **must** block by default
- [ ] The gate **must** fail closed on engine error, unreachable bundle, or evaluation timeout
- [ ] The policy source **must** sit behind CODEOWNERS with a security reviewer
- [ ] Thresholds **should** live in policy data keyed by reachability, not hardcoded in rules
- [ ] Exceptions **must** carry a rule id, an owner, and an expiry, and **should** live in one
  register
- [ ] Expired exceptions **must** stop suppressing, verified by a test
- [ ] Decision logs from every enforcement point **should** land in a common store
- [ ] The exception register **should** be reviewed on a fixed cadence, not when it hurts
- [ ] Chronically renewed exceptions **should** be treated as broken rules, not accepted risk
- [ ] Rules that have not fired in a year **should** get an explicit keep-or-retire decision
</Checklist>
