...
The earlier stages evaluate things the team controls directly. From here the artifact leaves
those systems, and the gates read signed evidence about it.
...
and not in the repository, so this stage uses mode 2 and requires a second collector. The
...
The SBOM is what answers whether a release is affected when a CVE is disclosed against a
dependency, which may be years after the build.

Most teams emit nothing here that anything downstream reads. The package is published, the
record stops, and no installer is in a position to notice.
...
as it likes and nothing prevents a different artifact from reaching production under the same
version number.
...
is what allows responders to determine what was running at a given point in time during a
...
- [ ] Signing **must** be gated on the build policy result, checked before signing runs
- [ ] Publishing identities **must** be org-owned, MFA-enforced, and resolved against the
  current roster
- [ ] Signing roles **should** hold multiple keys under threshold or quorum trust
- [ ] Online keys **should** not be the keys clients ultimately trust; unavoidable ones **must**
  be HSM-backed
- [ ] The published digest **must** equal the attested digest
- [ ] Registry version immutability **must** be enabled
- [ ] Provenance, SBOM, and signature bundle **must** be published and bound to the same digest
- [ ] The signature **must** be verified before any attestation claim is evaluated
- [ ] Provenance **must** be matched against builder identity, source repository, and ref
- [ ] Missing evidence **must** produce refusal
- [ ] An attested vulnerability scan **must** be required, with a tier-specific age threshold
- [ ] Deployable content **must** be scanned for secrets; dependency review **should** precede
  promotion
- [ ] The deployment identity **must** be short-lived and environment-scoped; production
  **must** require approval
- [ ] Admission decisions **must** be logged with rule ids, verdict, digest, and evidence
  consulted
- [ ] The deployed digest **must** be pinned in the manifest, never a mutable tag
- [ ] Drift **should** be detected continuously, with auto-resync or alerting
- [ ] Manual runtime mutation **must** be blocked; rollbacks **must** revert declared state
- [ ] Release data **should** be preserved: module versions, configuration, operational metadata
</Checklist>
