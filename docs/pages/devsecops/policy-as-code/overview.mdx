...
- [Open Policy Agent (OPA)](https://www.openpolicyagent.org/): a policy engine that
  streamlines policy management across a stack for improved development, security, and
...
enable. The question here is how a team confirms they are still enabled next quarter, across
every repository, without manual review.
...
Two things have to come from the adopting organization.

- The input structure depends on which SCM, registry, and CI platform the organization runs and
  on how its collectors shape their output. The field names will differ.
- The rule content is a statement of what the organization has decided to require, which is a
  different question from what a standard permits. Deriving those requirements from a team's own
