...
while the check fails. The enforcement point is a platform feature to configure; the decision
behind it belongs to the organization.
...
- the runner is on a maintained allowlist
- the job does not run privileged
- the runner is ephemeral and re-provisioned per job

The allowlist is policy data (`data.approved_runners`). It describes a specific infrastructure
rather than a general security property, and will differ between organizations.
...
A tag is a mutable pointer. It can be moved to different content without any change in the
consuming repository, so a build that passed previously can produce different output from
identical source.
...
build logs. Over 23,000 repositories used the action. An estimated 218 leaked secrets,
most of them short-lived workflow tokens
([CVE-2025-30066](https://nvd.nist.gov/vuln/detail/CVE-2025-30066)).
Repositories that pinned the action by commit SHA were not affected by the tag manipulation.
...
- [ ] Commit signatures **must** be verified, not merely required
- [ ] Signing identities **must** resolve against the current roster
- [ ] Protected paths **must** require their owning team
- [ ] Secret scanning **must** run at push and in CI, and the repository **should** have been
  scanned before it held code
- [ ] SCA **must** resolve the full transitive graph, and truncation **must** be treated as a
  failure
- [ ] Analysis **must** cover every language in use, verified against an expected set
- [ ] Commit-stage results **should** be recorded and keyed to the commit SHA
- [ ] Self-approval **must** be blocked; approval count **should** be set per tier
- [ ] Required checks **must** bind to the head SHA, not the branch
- [ ] Workflow definitions **must** be evaluated by policy before they execute
- [ ] `pull_request_target` **must** never be combined with an untrusted checkout
- [ ] Fork workflows **must** be either sandboxed or held for maintainer approval
- [ ] SCM posture **should** be assessed on a schedule and recorded
- [ ] Runners **must** be restricted to an approved allowlist, non-privileged and ephemeral
- [ ] Images **must** be pinned by digest; toolchains, lockfiles, and actions **must** be pinned
  immutably
- [ ] Release builds **must** be restricted to protected refs under a purpose-scoped identity
- [ ] Egress **should** be default-deny with a documented allowlist
- [ ] Attestation **must** be emitted, signed, and stored tamper-proof
- [ ] Attestation **must** be generated by a process more trusted than the build it describes
</Checklist>
