...
Advanced users, developers, and organizations seeking programmable security and customizable transaction policies, that don't want to (EVM, Solana, Sui) or can't (Bitcoin, Litecoin, Dogecoin, XRP) make fully customizable logic on each chain individually. Additionally, encumbered wallets can run within an encrypted runtime thus providing a level of transaction privacy.
...
This hardware-level protection makes TEE-based encumbered wallets significantly more secure than software-only solutions, as even root access to the host system cannot compromise the keys or policies within the TEE.
...
* Migrate to or support next-generation TEE environments that provide memory integrity protection such as Intel TME-MK, AMD SEV-SNP with enhanced integrity, ARM Confidential Compute Architecture (CCA), or Intel TDX. These newer implementations offer better protection against memory aliasing attacks and provide hardware-level integrity guarantees that older TEE implementations lack.
