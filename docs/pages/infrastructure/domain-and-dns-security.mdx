...
> This page is now an overview. See the subpages in this section for details:
...
### 1. DNSSEC Implementation (summary)
...
**Setup process (practical)**:
1) **Enable signing at the DNS provider**
   - Turn on DNSSEC in zone settings. Provider generates KSK/ZSK, signs the zone, and shows DS parameters:
     - Key Tag
     - Algorithm (commonly 13 ECDSAP256SHA256; sometimes 8 RSASHA256, 15 ECDSAP384SHA384)
     - Digest Type (2 SHA-256; sometimes 4 SHA-384)
     - Digest (hex)
2) **Publish DS at the registrar**
   - Create a DS with the exact four fields above. Prefer Algorithm 13 with Digest Type 2 unless registrar only supports others.
   - If both 2 and 4 are offered, pick 2 for widest compatibility.
   
   Registrar DS template
...
3) **Verify validation path**
   - After propagation, validate from a validating resolver. Use `dig +dnssec`, `kdig`, or web tools:
     - https://dnssec-analyzer.verisignlabs.com/yourdomain.org
     - https://dnsviz.net/d/yourdomain.org/analyze/
4) **Monitor DNSSEC validation** regularly and set alerts on DS changes/DNSKEY rollovers.

Security posture notes
- DNSSEC authenticates data; it does not encrypt queries. Use DoT/DoH for transport privacy when needed.
...
#### MTA-STS (Mail Transfer Agent Strict Transport Security) — summary
MTA-STS enforces encrypted connections between mail servers, preventing downgrade and man-in-the-middle attacks on SMTP transport.
...
1. Understand purpose: enforce TLS, prevent downgrade/MitM on SMTP.
2. Create policy file over HTTPS at: https://mta-sts.<your-domain>/.well-known/mta-sts.txt
   - Basic structure:
   ...
   - Example:
     ...
     max_age: 86400
3. Host the policy via HTTPS (valid cert) — e.g., your web host, Cloudflare Pages/Workers, S3+CloudFront.
4. Publish DNS TXT at `_mta-sts.<your-domain>` with value `v=STSv1; id=<policy-version-timestamp>`.
5. Start with `mode: testing`, review reports (if enabled), then move to `enforce`.
6. Ensure all MX hosts present valid TLS certificates matching hostnames. `max_age` controls cache duration.
...
**Setup**: Add a DMARC record to your DNS zone (prioritize aggregate reports; failure reports are rarely supported and raise privacy concerns):
```
...
#### Registry Lock (EPP Lock) — summary
...
**EPP Status Codes** that protect your domain (apply to registry objects like nameserver sets and contacts, not DNS zone edits at your DNS hosting panel):
- `clientTransferProhibited`: Prevents domain transfers to another registrar
- `clientUpdateProhibited`: Prevents updates to registry objects (not provider DNS records)
...
**How it protects you**: Standard transfer locks only prevent transfers between registrars. Registry locks with server-level protections block transfers/deletions and critical registry updates, but they do not block record edits in your DNS provider panel. Server-level locks typically require manual verification with the registry operator (for example, Verisign for .com), greatly reducing the impact of registrar-level social engineering.
...
- Strongly prefer **Hardware Security Keys (FIDO2/WebAuthn, e.g., YubiKey)** — phishing resistant
- **Avoid SMS 2FA** — susceptible to SIM swap
- Use **TOTP** only as a fallback when hardware keys are not possible
...
#### WHOIS Privacy Protection — summary
...
**Important**: WHOIS is legacy; prefer **RDAP** for structured, standardized registration data. WHOIS privacy does not affect legal ownership — it shields contact data from public view. Example RDAP lookup tools and CLI clients provide richer outputs and links to registrar/registry RDAP endpoints.
...
### 1. DNS Record Monitoring — summary
...
- **TTL values dropping very low**: Low TTL is common with CDNs/auto-scaling; not inherently malicious, but sudden drops can precede rapid changes. Note that some providers use fixed TTLs (for example, Cloudflare “Auto” = 300s for many records).
- **CAA record removal or override**: Parent CAA is overridden by child zones; parameters like `accounturi` and `validationmethods` can provide finer control.
...
- [SecurityTrails](https://securitytrails.com/) - Change tracking and historical DNS

**GitOps/Zone control tooling** (auditable changes):
...
- GoDaddy incident coverage: https://threatpost.com/godaddy-employees-tricked-compromise-cryptocurrency/161520/
- Low-tech high-impact breaches: https://krebsonsecurity.com/2023/02/when-low-tech-hacks-cause-high-impact-breaches/
