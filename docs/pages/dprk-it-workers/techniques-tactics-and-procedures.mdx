...
1. There is no single established channel through which a DPRK IT Worker can approach your company.
    ...
    1.1 **Direct outreach to CEO/CTO.** They are persistent until clearly denied the opportunity, and even then,
        the actor may change their identity and try again.
    1.2 **Initiating contact through open-source contributions.** An actor will submit valid Pull Requests to your
        repositories and use this as a way to request a job interview.
    1.3 **Recommendation.** A worker will come recommended by another DPRK IT Worker or by a company they previously
        infiltrated (where the company may or may not be aware the employee is a DPRK IT Worker).
    1.4 **Tech recruiters.** Recruiters often prioritize evaluating skillsets over performing background checks.
        DPRK IT Workers can appear on their radar and be unwittingly pushed by a legitimate tech recruiter to your company.
    1.5 **Bounty/Hackathon participation.** DPRK IT Workers look for ANY source of revenue, no matter how small or
        large. If your company runs free-for-all bounty systems, it's extremely likely that some of the contributors are
        DPRK IT Workers.
    1.6 **Hired through a "dev shop."** If you recruit your employees through an outsourcing agency, you need to
...
3. Below is a list of 'red flags' that can help you evaluate with higher confidence if you are dealing with a DPRK IT
    ...
    3.1 **Is their video on?** If not, insist they enable their camera.
        3.1.1 Some DPRK IT Workers have no problem presenting as themselves (without a facilitator, AI, or disabled
            video). See if you recognize anyone from this
            [Collection of DPRK IT Workers photos](https://x.com/browsercookies/media).
    3.2 **Is the background visible or obscured?** An obscured background isn't always a red flag, but it can be
        helpful for the final assessment.
        3.2.1 DPRK IT Workers love "stock" backgrounds. You'll often find them using the Golden Gate Bridge as a
    ...
    3.4 **Start with small talk** relevant to their claimed nationality and location. This is especially important if
    ...
    3.5 **Keep an open mind** about gender, nationality, and experience. **DPRK IT workers are known to use
        ...
        organizations.
    3.6 **Go through the candidate's background.** Verify the timelines of their claimed experience (e.g., "You worked
    ...
    3.7 **Conduct a language comprehension test.** If the candidate claims to be Japanese, ask them questions in
    ...
    3.8 **Ask if the candidate is at home or a co-working space.**
        3.8.1 Look for movement in the background.
        3.82 If they claim to be at home, you can try a quick KYC check: **ask them to show their ID card next to
            their face.**
    3.9 **Carefully observe the candidate's screen presentation if they share their screen.** Often, you might notice
    ...
    3.10 **Evaluating a potential DPRK IT Worker's GitHub profile:**
        3.10.1 The account creation date is later than the commits in its hosted repositories (an artificially aged
        ...
        3.10.2 A high number of followers and accounts followed (e.g., over 200). If the user is a popular developer
        ...
        3.10.3 A high number of contributors to the user's repositories. If the user is not a popular developer, this
        ...
        3.10.4 Quality of opened Pull Requests and Issues. **It's useful to retrieve the full history of Pull Requests
        ...
        3.10.5 **Overly informative (full of images and stats) GitHub profile README/About page.**
            3.10.5.1 This can be a false positive. Regular developers have been observed to overuse this section.
        3.10.6 **Avatars.**
            3.10.6.1 DPRK IT workers often use AI-generated images (not necessarily faces) as their avatars.
                They typically use freely accessible models like Stable Diffusion or Midjourney.
            3.10.6.2 "Cartoonish" avatars are also popular: CGI-style images of fictional characters, Minion avatars,
                and NFT-related avatars (especially Pengu and Doodles).
            3.10.6.3 **Having no avatar at all is also common.**
            3.10.6.4 Reference:
                [Identifying Suspicious Github Accounts](https://www.ketman.org/identifying-suspicious-gh-accounts.html)
        3.10.7 **Social media links.**
            3.10.7.1 In many cases, there will be a Twitter link. It's always worth visiting.
                3.10.7.1.1 Check the "Media" page for images uploaded by the account owner. **Can you find any
                ...
                3.10.7.1.2 Look for job-begging tweets.
                3.10.7.1.3 References:
                    3.10.7.1.3.1 [Nick Franklin Case - A well developed IT Worker
                        persona](https://x.com/tanuki42_/status/1905003045433290940)
                    3.10.7.1.3.2 [Nick Franklin Case - Activity across different
                        platforms](https://x.com/danielvf/status/1905642180749775189)
        3.10.8 **Suspicious interactions.**
            3.10.8.1 Contributions to low-quality external repositories and/or organizations. **DPRK IT workers are
                ...
                3.10.8.1.1 Reference:
                    [A counter example - Good looking IT Worker Github
                    Profiles](https://x.com/blackbigswan/status/1873364567285449160)
        3.10.9 **Repositories.**
            3.10.9.1 "Copied repositories." DPRK IT workers will try to boost their credibility by pushing a clone
            ...
            3.10.9.2 GitHub's Activity Badge for contributing to an organization. Verify if it's an actual
                ...
                spam PRs.
                3.10.9.2.1 Reference: [DPRK IT Workers faking Github Activity
                Badges](https://x.com/blackbigswan/status/1876953373187920054/photo/1)
    3.11 **Evaluating other immediately available data points:**
        3.11.1 **Full Name.**
            3.11.1.1 Even if a full name is "fake," it can still be a useful data point. DPRK IT workers rotate their
                ...
                3.11.1.1.1 Take extra care with Asian names, especially Chinese names. Chinese citizens often
                    westernize or shorten their full names in various ways. This doesn't necessarily mean they are
                    malicious.
            3.11.1.2 In most cases, DPRK IT workers prefer to use Asian (Japanese, Chinese, Korean) names. However,
                ...
                3.11.1.2.1 DPRK IT workers may have access to KYC documents issued for their claimed nationality.
        ...
        3.11.2 **Location.**
            3.11.2.1 Japan and the USA are extremely popular choices for claimed locations. Additionally, Southeast
        ...
        3.11.3 **OSINT analysis.**
            3.11.3.1 We recommend checking each suspicious email address with a service like
                ...
                3.11.3.1.1  On LinkedIn, examine the strength of the actor's connection network.
...
1. The list below serves as a guide for confirming your suspicions if one of your employees is a potential
    ...
    1.1  If your organization has EDR/SIEM capability, Read:
        [DTEX advisory on DPRK Insider Threat Kill
        Chain](https://www.dtexsystems.com/resources/i3-threat-advisory-inside-the-dprk/)
    1.2  Consider using GitHub Enterprise for your organization to have access to more detailed logs.
...
3. Below is a list of 'red flags' that should prompt immediate action from your side (e.g. a deep background check):
    3.1  **Avoiding real-life meetings.** Even if they initially agree to meet, they might come up with explanations
        ...
        test.**
    3.2  **Inconsistencies in work and communication.** It's often the case that a single DPRK IT Worker account is
    ...
    3.3  **Timezone.** If your worker claims to be in Texas (GMT-5) but is located in Pyongyang (GMT+9), you may
    ...
    3.4  **Bogus work history.** Verify if the job history claimed on their CV is real. You can always reach out to
    ...
    3.5  **Identity switch.** Be on guard if an employee's profile suddenly changes its nickname/identity.
    ...
    3.6  **Identity mismatch.** It should raise questions if your employee is Asian but provides non-Asian KYC
    ...
    3.7  **Recommending other suspicious accounts for work.** DPRK IT Workers will leverage their initial foothold
    ...
    3.8  **Proximity to other suspicious/spam accounts.** Don't be fooled by GitHub or Twitter accounts that are over
        ...
        like bots?
    3.9  **Poor social skills.** It's usually (but not always) the case that a DPRK IT Worker will have trouble with
    ...
    3.10 **Review Workspace logs (IP addresses).** Check if the IP addresses used to log in to your infrastructure
    ...
    3.11 **Payments.** North Koreans can organize a 'drop' bank account to receive wire transfers; however, their
...
1. **The main goal of a DPRK IT Worker is to keep their job and salary coming for as long as possible.** It's worth
    ...
    performance.
    1.1  We have observed cases where DPRK IT Workers are fine with taking a salary cut to help a project stay afloat,
