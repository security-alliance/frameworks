...
- [ ] User Settings > Account > Password & Security > Multi-Factor Authentication: Ensure **2FA** is enabled
  (authenticator app and/or security key), Remove a phone number if you have one added to your account, and
  after 2FA is setup select **View Backup Codes**, and note down your backup codes offline
- [ ] User Settings > Account > Account Info: Ensure **SMS Backup Authentication** is **disabled**
- [ ] User Settings > Messaging Permissions > Direct Messages > Direct Message (DM) Permissions: Allow DMs
  from other server members > **Disabled**, Filter messages from server members I may not know > **Disabled**
  (encourages moderators and community members to adopt the same setting to minimize phishing DMs)
- [ ] User Settings > Messaging Permissions > Spam Filters: Select **Filter all spam** to filter all DMs for spam
- [ ] User Settings > Connected Apps > Authorized Apps: Review and **Deauthorize** any unnecessary apps
- [ ] User Settings > Account > Password & Security > Logged-in Devices: Review and remove unnecessary devices, 
  or **Log Out All Known Devices**
- [ ] User Settings > Connected Apps > Connections: Review and remove any unnecessary connections
</Checklist>
...
- Safety Setup > Moderation > Permissions:
<Checklist id="safety-setup">
  - [ ] Require 2FA for moderation > **Enabled**
  - This ensures all moderators have an extra layer of security
  - [ ] Restrict prune members to admin > **Enabled**
  - [ ] Remove risky permissions for @everyone > **Enabled**

</Checklist>
- Safety Setup > DM and Spam Protection > Verification Level:
<Checklist id="safety-setup-2">
...
<Checklist id="community-features">
...
<Checklist id="community-features-2">
...
particular rules. [Beemo](https://beemo.gg/) is a good example of a bot in this category.
...
creation/deletion. A popular bot in this category is [Wick Bot](https://wickbot.com/).

#### Elevated Permissions & Link Allowlisting Bots

These bots enforce least privilege by requiring a successful 2FA session before elevated permissions are granted.
These bots only allow approved links to be sent in a Discord server, all other links are blocked with admins being able
to manage an allowlist. Popular bots in this category are [ZTS Guard Bot](https://zts-guard.gitbook.io/zts-guard-discord-bot)
or [Goodknight Bot](https://docs.goodknightbot.com/).

#### General Moderation Bots

General moderation settings include logging on server actions, anti-webhook enforcement, extending security to DMs outside of the Discord server, and other discord settings that can be enforced from these bots. Consider bots like [Dyno](https://dyno.gg/) or [ZTS Guard Bot](https://zts-guard.gitbook.io/zts-guard-discord-bot) for advanced moderation and logging, or [Carl-bot](https://carl.gg/) for reaction roles and custom commands. Set up security Bots as described above.
...
<Checklist id="step-1-create-a-dedicated-email-account">
...
<Checklist id="step-2-create-the-discord-account">
...
<Checklist id="step-3-join-the-server-and-transfer-ownership">
