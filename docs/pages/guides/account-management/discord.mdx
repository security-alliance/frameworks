...
#### Moderation & Link Allowlisting Bots

These bots enforce least privilege by requiring a successful 2FA session before elevated permissions are granted. These bots only allow approved links to be sent in a Discord server, all other links are blocked. Popular bots in this category are [ZTS Guard Bot](https://zts-guard.gitbook.io/zts-guard-Discord-bot) or [Goodknight Bot](https://docs.goodknightbot.com/).
