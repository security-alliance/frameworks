...
- [ ] User Settings > Account > Password & Security > Multi-Factor Authentication: Ensure **2FA** is enabled (authenticator app and/or security key), Remove a phone if you have one added to your account, and after 2FA is setup select **View Backup Codes**, and note down
...
- [ ] User Settings > Messaging Permissions > Direct Messages > Direct Message (DM) Permissions: Allow DMs from other server members > **Disabled**, Filter messages from server members I may not know > **Disabled**
...
- [ ] User Settings > Account > Password & Security > Logged-in Devices: Review and remove unnecessary devices, or **Log Out All Known Devices**
...
These bots enforce least privilege by requiring a successful 2FA session before elevated permissions are granted. These bots only allow approved links to be sent in a Discord server, all other links are blocked with admins being able to manage an allowlist. Popular bots in this category are [ZTS Guard Bot](https://zts-guard.gitbook.io/zts-guard-Discord-bot) or [Goodknight Bot](https://docs.goodknightbot.com/).
