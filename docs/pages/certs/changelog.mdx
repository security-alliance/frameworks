...
- **[SFC - Identity & Accounts](/certs/sfc-identity-accounts)** (v1.0): new horizontal cert covering organizational account management — inventory (social media, email, SSO, registrar, custody, repo admin, cloud root, SaaS), phishing-resistant MFA, password manager with individual accountability, recovery methods restricted to org channels, account lifecycle, takeover monitoring, third-party access. Addresses a gap where org-account takeover (Twitter, status page, registrar) was not covered despite being a top-tier attack vector for crypto protocols.
...
- `di-1.1.2` supply-chain mention softened to cross-reference Section 2.
- `di-1.1.4` narrowed to the *evaluation process*.
- `di-1.1.5` (NEW): approved tools list, reviewed at least annually.
...
- References section added at the bottom of the cert with links to Cloud Security Alliance CCM and cloudsecdocs.com.
- Control count: 16 → 17.
...
- `ir-2.1.1` **(NEW)**: threat model for protocol operations, including external dependencies. Placed before monitoring coverage so monitoring is anchored to a known threat picture. Existing Section 2 controls shifted: previous `ir-2.1.1` → `ir-2.1.2`, `ir-2.1.2` → `ir-2.1.3`, `ir-2.1.3` → `ir-2.1.4`.
...
- Control IDs are stable across this revision. No renames. New controls (`tro-2.1.3`, `tro-3.1.5`, `di-1.1.5`, all `ida-*`) will simply be unpopulated when importing old workbooks, which is expected.
