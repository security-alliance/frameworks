...
- id: dns-3
  ...
  - id: dns-3.1.1
    description: Do you control and secure access to domain registrar and DNS
      management accounts?
    baselines:
      - "Registrar and DNS management accounts classified as high-privilege and
        managed per [SFC - Identity & Accounts](/certs/sfc-identity-accounts)
        (phishing-resistant MFA via hardware keys, credential management, recovery
        methods, account lifecycle, periodic access reviews)"
      - Registrar account ownership and role-based permissions documented where
        the registrar supports them
